        ...
        if content_type == 'text':
            ...
            if (msg_text.startswith('/')):
                ...
                if (command == 'start'):
                    ...
                    self.state = 4
                    bot.sendMessage(chat_id, 'Please enter your feedback: ')
                elif (command == 'rate'):
                ...
                with open('stalls.json') as f:
                    reader = f.read()
                    rlist = json.loads(reader)
                #check if user input for canteen is valid (matches with that in master list, stalls.csv)
                for i in range(len(rlist)):
                    if(msg_text == rlist[i]["Food Location"]):
                        self.check = 1
                ...
                if(self.check):
                    ...
                    for i in range(0, len(rlist)):
                        if(rlist[i]["Food Location"] == msg_text):
                            otpt = otpt + rlist[i]["Food Stalls"] + "\n"
                            self.stall.append(rlist[i]["Food Stalls"])
